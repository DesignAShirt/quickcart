...
(function (root, factory) {
  ...
  function Item(data) {
    ...
    var id, price, signature, properties, quantity;
    ...
    this.options = {
      ...
      signer: function(cart) {
        var itemsAsStr = _this._items.map(function(item) {
          return [ item.id, item.price, item.signature, item.quantity ].join(':');
        }).join('|');
        return hashCode([ _this.store, _this.user, itemsAsStr ].join(';'));
      },

      // payment processor strategy
      processor: function(cart, callback) {
        throw new Error('No payment processor strategy provided');
      }
    };
  ...
  Cart.prototype.watchItem = function(item) {
    var _this = this;
    item
  ...
  Cart.prototype.add = function() {
    var additions = toArray(arguments)
      , addition
      , item;
  ...
  Cart.prototype.remove = function() {
    var removals = toArray(arguments)
      , removal
      , item;
  ...
  Cart.prototype.toJSON = function() {
    ...
    opts.signature = this.signature;
    opts.meta = {};
    opts.items = this._items.map(Function.prototype.call, Item.prototype.toJSON);
    return opts;
  };
