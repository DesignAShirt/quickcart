...
(function (root, factory) {
  ...
  Cart.prototype.update = function() {
      if (this.locked) {
          throw new Error('Cannot update because cart is locked');
      }
      var updates = toArray(arguments, true)
          , update
          , countChanged = false
          , qtyChanged = false
          , totalChanged = false
      ...
      if (updates.length) {
          for (var i=0; i < updates.length; i++) {
              update = updates[i];
              item = update instanceof Item ? this.find(update.id) : null;
              if (item) {
                  var index = this._items.indexOf(item);
                  this._items.splice(index, 1);
                  this._items.splice(index, 0, update);
                  this._watchItem(update);
                  this.trigger('item:change', update);
                  this.trigger('change');
                  countChanged  = countChanged  || Item.effectsCount(update);
                  qtyChanged    = qtyChanged    || Item.effectsQuantity(update);
                  totalChanged  = totalChanged  || Item.effectsTotal(update);
              }
              else {
                this.trigger('error', new Error('Item "' + update.id + '" was not found in cart'));
              }
          }
          countChanged  && this.trigger('count');
          qtyChanged    && this.trigger('quantity');
  ...
  Cart.prototype.remove = function() {
    ...
    if (removals.length) {
      for (var i=0; i < removals.length; i++) {
        removal = removals[i];
        item = removal instanceof Item ? removal : this.find(removal.id);
        if (item) {
